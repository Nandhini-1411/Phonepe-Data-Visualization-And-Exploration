...
if selected_page == "Home":
    ...
    """)
    with col22:
        st.image(r"D:\CAPSTONE\PHONEPE\IMAGES\phonpe1.jpg",use_column_width=True)
    with col22:
        st.image(r"D:\CAPSTONE\PHONEPE\IMAGES\phonepe3.png")
        col1,col2,col3 = st.columns(3)
        with col2:
            st.image(r"D:\CAPSTONE\PHONEPE\IMAGES\phonemap1.jpg")
    ...
    st.write("""
    Explore the power of data visualization and uncover actionable insights into PhonePe's performance and user behavior. Use the option-menu above to navigate through different sections, apply filters, and interact with visualizations to delve deeper into your data. Start exploring now!
    """)
if selected_page == "State-Wise Analysis":
    ...
    if selected_option=="User Analysis":
        agg_user_data_path = r"D:\CAPSTONE\PHONEPE\data\aggregated\user\state/"
        ...
        if filtered_Agg_Users_1["Device_Brand"].isnull().any():
            st.error("Oops! Sorry. There is No Device Brand Details Available for Some Entries in the Filtered Data.")
        ...
        st.markdown('<hr>', unsafe_allow_html=True)
        st.subheader("YEAR WISE HIGHEST REGISTRATION COUNT STATE")
    ...
    if selected_option == "Transaction Analysis":
        agg_transaction_data_path = r"D:\CAPSTONE\PHONEPE\data\aggregated\transaction\state/"
    ...
    if selected_option == "Insurance Analysis":
        agg_insurance_data_path = r"D:\CAPSTONE\PHONEPE\data\aggregated\insurance\state/"
...
if selected_page == "Interactive Map Viewer":
    ...
    conn = fun.create_connection()
    if selected_option=="User Analysis":
        map_user_data_path = r"D:\CAPSTONE\PHONEPE\data\map\user\state/"
        ...
        fig = px.choropleth(df, 
                        ...
                        labels={"avg_registered_users": "Avg Registered Users", "avg_app_opens": "Avg App Opens"},
                        width = 1400,height = 1000)
        fig.update_geos(fitbounds="locations", visible=False)
        st.plotly_chart(fig)
    if selected_option == "Transaction Analysis":
        map_trans_data_path = r"D:\CAPSTONE\PHONEPE\data\map\transaction\state/"
        ...
        fig = px.choropleth(df1, geojson=geojson_data, locations="state", featureidkey="properties.NAME_1",
                    ...
                    labels={"Avg_Transaction_Count": "Avg Transaction Count", "Avg_Total_Transaction_Amount": "Avg Transaction Amount"},
                    width = 1400,height = 1000)
        fig.update_geos(fitbounds="locations", visible=False)
        st.plotly_chart(fig)
    if selected_option == "Insurance Analysis":
        st.caption("Please Select Year and Quarter From 2020-2nd")
        map_ins_data_path = r"D:\CAPSTONE\PHONEPE\data\map\insurance\state/"
        ...
        fig = px.choropleth(df2, geojson=geojson_data, locations="state", featureidkey="properties.NAME_1",
                    color="Avg_Total_Count", color_continuous_scale="Sunsetdark",
                    range_color=(df2["Avg_Total_Count"].min(), df2["Avg_Total_Count"].max()),
                    hover_name="state",hover_data={"state": True, "Avg_Total_Count": True, "Avg_Total_Amount": True}, 
...
if selected_page == "District-Wise Analysis":
    ...
    selected_option= st.radio("Select:",["User Analysis","Transaction Analysis","Insurance Analysis"])
    if selected_option == "User Analysis":
        top_user_data_district_path = r"D:\CAPSTONE\PHONEPE\data\top\user\state/"
    ...
    if selected_option == "Transaction Analysis":
        top_transaction_district_data_path = r"D:\CAPSTONE\PHONEPE\data\top\transaction\state/"
    ...
    if selected_option == "Insurance Analysis":
        top_insurance_district_data_path = r"D:\CAPSTONE\PHONEPE\data\top\insurance\state/"
